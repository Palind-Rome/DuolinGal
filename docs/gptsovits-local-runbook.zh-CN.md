...
当这条最小复现流程进一步扩大到“全角色夜跑量产”后，我们又在工程侧补了几层保护：

1. 纯标点英文句会在批次准备阶段直接跳过
   例如 `......`、`.` 这类行，不再送去 `/tts`。
2. 如果个别句子在 GPT-SoVITS 内部进一步清洗后变成无效文本
   队列会记录并跳过这一句，而不是整晚直接停掉。
3. Windows 下如果 `wav -> ogg` 因为日文路径导致 `soundfile` / `ffmpeg` 读取失败
   会先走一次 ASCII 临时中转，再完成转换。
4. 量产队列现在会持续写出：
   - 当前角色
   - 当前阶段
   - 当前已完成句数 / 总句数
...
所以当前仓库的“量产入口”比这份最小复现文档里的最初版本要更稳，也更适合长时间无人值守。
