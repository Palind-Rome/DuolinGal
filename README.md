...
这套工作流里，**在《千恋＊万花》这条已经跑通的产线中可以直接确认用到的核心工具**主要有：

- [GPT-SoVITS](https://github.com/RVC-Boss/GPT-SoVITS)
  负责角色语音克隆、单角色训练、多角色推理，是整条语音生成链的核心。
- [FreeMote](https://github.com/UlyssesWu/FreeMote)
  负责反编译和重建 `.scn`、`.psb`、`.psb.m` 等 KiriKiri 常见资源；当前项目日志里可以直接看到它的实际调用记录。
- [KirikiriTools](https://github.com/arcusmaximus/KirikiriTools)
  负责 `patch2.xp3` 打包；本地 PoC 调试时用到的 `version.dll` 也来自这个项目。
- [FFmpeg](https://ffmpeg.org/)
  负责音频裁切、重采样、格式转换和 `wav -> ogg` 转码。它不是 GitHub 仓库型项目，但同样是这条工作流的重要基础设施。

仓库当前还**支持或预留**了以下前置/可选工具：

- [KrkrDump](https://github.com/crskycode/KrkrDump)
  用于为 KiriKiri Z 游戏准备运行时 dump 配置；仓库里已经集成了 `prepare-krkrdump` 命令和相关检查逻辑。
- [KrkrExtract](https://github.com/xmoezzz/KrkrExtract)
  可选使用，用于离线解包或重打包 XP3 资源。

另外说明一下：

- [GARbro](https://github.com/morkt/GARbro) 当前**不是** DuolinGal 主工作流里的直接集成工具。
- 它目前只在可行性文档里作为社区资料和兼容性参考被提到，不属于这次《千恋＊万花》量产链的核心依赖。
