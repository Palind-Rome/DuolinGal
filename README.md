...
- 已完成最小 CLI：`analyze`、`init-project`、`list-tools`、`extract`、`build-lines`
- 已完成最小本地 API：健康检查、工具探测、目录分析、项目初始化、资源提取、`lines.csv` 构建
- 已完成工具链配置读取、进程执行封装和提取日志落盘
...
- 按你的本地工具路径修改
- `krkrextract.args` 里的 `{package}`、`{output}`、`{workspace}` 是模板变量
- `KrkrExtract` 的具体命令行参数格式以你的版本为准，示例配置只演示模板替换方式
